...
### PHASE_CHECKPOINT 19 — Manuel PAPER/TESTNET Bot Kotası Düzeltmesi

- Kök neden: Arena sermaye alanı, frontend şeması ve backend servisinde bot başına `200 USDT` sabit üst sınırı vardı. `100` kotası olan botta alana `500` yazıp “Bakiye ekle” seçmek hedefi `600` yaptığı için istek doğrulamada reddediliyordu; bağlantı hatası değildi.
- Değişiklik: PAPER/DEMO autonomous bot uygulama kotası üst sınırı `10.000 USDT` yapıldı. “Kotayı ayarla” toplam kotayı, “Bakiye ekle” mevcut kotaya eklenecek miktarı ifade edecek şekilde UI açıklaması ve istemci ön doğrulaması güncellendi. Bootstrap/deploy scriptleri 200 üzerindeki manuel kotaları artık 100'e sıfırlamaz.
- Risk: TESTNET kotası Binance Demo cüzdanına para eklemez; botun ortak demo bakiyesinden kullanabileceği uygulama notional sınırını artırır. Transaction aynı anda hesap ve sembol risk profilini yalnız gereken yeni toplam kotaya yükseltir; merkezi Risk Engine, isolated margin, stop/TP ve production LIVE kapısı korunur.
- Stop incelemesi: Mevcut TESTNET fleet `20–40 bps` (`%0,20–%0,40`), PAPER varsayılanı `50 bps` (`%0,50`) stop kullanıyor; bunlar piyasa gürültüsü ve maliyetler için dar olabilir. Kullanıcının önerdiği sabit `%20`, 5x–20x kaldıraçta likidasyon mesafesinin arkasında kalabileceğinden uygulanmadı. Sonraki çalışma ATR + kaldıraç + liquidation-buffer tabanlı adaptif stop ve walk-forward karşılaştırmasıdır.
