...
import { getTradingExecutionProfile, updateTradingExecutionProfile, type TradingExecutionProfile } from '../../../../src/services/tradingService';
...
const fields = [
  ...
  ['takeProfitBps', 'Net kâr hedefi (bps; 100 = %1)', 10, 5000, '1'],
...
export function ExecutionSettingsPanel({ accountId }: { accountId: string }) {
  ...
  function accept(value: TradingExecutionProfile) {
    setProfile(value);
    setDraft(Object.fromEntries(fields.map(([key]) => [key, String(value[key])])));
  }
  useEffect(() => {
    let active = true;
  ...
  async function save(toggle = false) {
    ...
    const payload = toggle ? { entryPaused: !profile.entryPaused } : Object.fromEntries(fields.map(([key]) => [key, typeof profile[key] === 'number' ? Number(draft[key]) : draft[key]]));
