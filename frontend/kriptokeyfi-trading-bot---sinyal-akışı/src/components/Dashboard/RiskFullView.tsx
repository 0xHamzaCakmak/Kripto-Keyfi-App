...
import { aiTradingApi, type TradingRiskProfile, type TradingRiskEvent } from '../../../../src/services/aiTradingService';
...
export function RiskFullView({ accountId, accountName }: { accountId: string | null; accountName?: string }) {
  return <div id="risk-full-view" className="space-y-5">
    <header className="rounded-2xl border border-[#2b3139] bg-[#1e2329] p-5">
      <h1 className="flex items-center gap-2 text-xl font-bold"><ShieldAlert/> Risk Yönetimi</h1>
      <p className="mt-2 text-sm text-[#848e9c]">{accountName ?? 'Hesap seçilmedi'} · İşlem ve risk ayarları üstte seçilen hesap için burada yönetilir.</p>
    </header>
