...
import { ArrowDownRight, ArrowUpRight, Check, Layers, LoaderCircle, RefreshCw, ShieldAlert, Sparkles } from 'lucide-react';
...
import { DemoAccountSummary } from './DemoAccountSummary';
import { getCoinIcon } from '../CoinIcons';

type Props = { accountId: string | null; positions: TradeProPosition[]; operations: TradeProOperation[]; loading: boolean; error?: string; updatedAt?: string | null; onRefresh: () => void };
type CloseScope = 'ALL' | 'PROFITABLE' | string | null;

export const PositionsFullView: React.FC<Props> = ({ accountId, positions, operations, loading, error, updatedAt, onRefresh }) => {
  const [selectedKey, setSelectedKey] = useState<string | null>(null);
  const selected = positions.find((position) => position.positionKey === selectedKey);
  const [working, setWorking] = useState<CloseScope>(null);
  const [notice, setNotice] = useState<{ tone: 'success' | 'error'; text: string } | null>(null);

  const rows = useMemo(() => positions.filter((position) => Math.abs(Number(position.quantity)) > 0), [positions]);
  ...
  async function closePositions(targets: TradeProPosition[], scope: Exclude<CloseScope, null>, confirmation: string) {
    if (!accountId || targets.length === 0 || working || loading || error || !window.confirm(confirmation)) return;
    setWorking(scope); setNotice(null);
    const closed: string[] = [];
    const failed: string[] = [];
    for (const position of targets) {
      try { await closeTradeProPosition(accountId, position); closed.push(position.positionKey); }
      catch (reason) { failed.push(`${position.symbol}: ${getApiErrorMessage(reason, 'Kapatma emri gönderilemedi.')}`); }
    }
    if (failed.length) setNotice({ tone: 'error', text: closed.length + ' kapatma isteği alındı; başarısız olanlar: ' + failed.join(', ') });
    else setNotice({ tone: 'success', text: closed.length + ' market kapatma isteği alındı. Sonuç borsa pozisyonları yenilenerek doğrulanıyor.' });
    // An accepted close request is not proof of a fill; keep the exchange rows.
    onRefresh();
    setWorking(null);
  }
  ...
  return <div id="positions-full-view" className="w-full space-y-5 animate-in fade-in duration-200">
    <div className="bg-[#1e2329]/90 border border-[#2b3139] rounded-2xl p-6 shadow-xl flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
      <div className="flex items-center gap-3.5"><div className="w-12 h-12 rounded-xl bg-[#0b0e11] border border-[#02c076]/40 flex items-center justify-center text-[#02c076] shadow-[0_0_15px_rgba(2,192,118,0.25)]"><Layers className="w-6 h-6" /></div><div><div className="flex items-center gap-2"><h1 className="text-xl sm:text-2xl font-bold tracking-tight text-[#eaecef]">Açık Pozisyonlar Yönetimi</h1><span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#02c076]/15 text-[#02c076] border border-[#02c076]/30">{rows.length} AKTİF İŞLEM</span></div><p className="text-xs text-[#848e9c] mt-0.5">Seçili demo borsasındaki tüm açık pozisyonlar; bot, manuel veya borsa üzerinden açılan işlemler dahildir.</p></div></div>
      <div className="flex flex-wrap items-center justify-end gap-3">
        <button type="button" disabled={!accountId || loading || Boolean(working)} onClick={onRefresh} className="flex items-center gap-2 rounded-xl border border-[#2b3139] px-3 py-2 text-xs text-[#00d2ff] disabled:opacity-40"><RefreshCw size={14} className={loading ? 'animate-spin' : ''}/> Yenile</button>
        ...
        <button type="button" disabled={!accountId || rows.length === 0 || Boolean(working) || Boolean(error) || loading} onClick={() => void closePositions(rows, 'ALL', 'Tüm açık pozisyonlar market fiyatından kapatılacak. Onaylıyor musunuz?')} className="flex items-center gap-1.5 px-4 py-2 bg-[#f84960]/20 hover:bg-[#f84960]/30 text-[#f84960] border border-[#f84960]/40 rounded-xl text-xs font-bold transition-all disabled:opacity-40">
    ...
    <div className="text-xs text-[#848e9c]">10 saniyede bir borsadan yenilenir. Son başarılı güncelleme: {updatedAt ? new Date(updatedAt).toLocaleString('tr-TR') : '—'}</div>
    {error && <div role="alert" className="rounded-xl border border-[#f84960]/40 bg-[#f84960]/10 p-3 text-xs text-[#f84960]">{error}{rows.length > 0 && ' Son başarılı okumadaki pozisyonlar gösteriliyor; veriler güncel olmayabilir.'}</div>}
    {accountId && <DemoAccountSummary key={`summary:${accountId}`} accountId={accountId}/>}
    {selected && accountId && <PositionCloseDialog key={selected.positionKey} accountId={accountId} position={selected} disabled={loading || Boolean(error) || Boolean(working)} onClose={() => setSelectedKey(null)} onSubmitted={() => { setNotice({ tone: "success", text: "Kapatma isteği alındı. Sonuç borsadan yenileniyor; limit emri gerçekleşene kadar pozisyon açık kalabilir." }); onRefresh(); }}/>}
    ...
    <div className="bg-[#1e2329]/80 border border-[#2b3139] rounded-2xl shadow-xl overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-xs border-collapse font-['JetBrains_Mono',monospace]">
      ...
      <tbody className="divide-y divide-[#2b3139]">{loading && rows.length === 0 ? <Empty text="Açık pozisyonlar backend üzerinden yükleniyor…" /> : error && rows.length === 0 ? <Empty text="Pozisyon bilgisi alınamadı. Yenile düğmesiyle tekrar deneyin." /> : rows.length === 0 ? <Empty text={accountId ? 'Bu hesapta açık pozisyon bulunmuyor.' : 'Aktif demo hesabı seçin.'} /> : rows.map((position) => <PositionRow key={position.positionKey} position={position} onAdvanced={() => setSelectedKey(position.positionKey)} operation={operations.find((item) => item.symbol === position.symbol && item.position?.side === position.side)} working={working === position.positionKey} disabled={!accountId || loading || Boolean(error) || Boolean(working)} onClose={() => void closePositions([position], position.positionKey, position.symbol + ' pozisyonu market fiyatından kapatılacak. Onaylıyor musunuz?')} />)}</tbody>
    </table></div></div>
  </div>;
};

function PositionRow({ position, operation, working, disabled, onClose, onAdvanced }: { onAdvanced: () => void; position: TradeProPosition; operation?: TradeProOperation; working: boolean; disabled: boolean; onClose: () => void }) {
