...
import { api, getApiErrorMessage } from '../../../../src/services/apiClient';
  ...
  return <div id="exchange-accounts-view" className="w-full space-y-5 animate-in fade-in duration-200">
    ...
    {loading ? <div className="grid gap-4 md:grid-cols-2">{[1,2].map((item) => <div key={item} className="h-56 animate-pulse rounded-2xl bg-[#1e2329]"/>)}</div> : accounts.length === 0 && !error ? (
      ...
      <article key={account.id} className="rounded-2xl border border-[#848e9c]/10 bg-[#1e2329] p-5 md:p-6">
        ...
        {!account.canTrade && <div className="mt-4 flex items-start gap-2 rounded-xl border border-[#f84960]/20 bg-[#f84960]/10 p-3 text-xs leading-5 text-[#f84960]"><AlertTriangle className="mt-0.5 shrink-0" size={16}/><span>Hesap okunabiliyor ancak Futures işlem yetkisi kapalı. Manuel emir için Binance Demo API anahtarında işlem/Futures yetkisini açın, ardından aşağıdaki “Bağlantıyı test et” butonuyla yetki durumunu yenileyin.</span></div>}
