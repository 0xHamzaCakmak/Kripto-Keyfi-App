...
import { latestPerSymbol } from './signalFlowState';
import { useFeedWindow } from './feedWindow';
...
export const SignalFeed: React.FC<SignalFeedProps> = ({ onSelectPair, arena, loading, hasBackendAccount }) => {
  const recentDecisions = useMemo(() => latestPerSymbol(arena?.recentDecisions ?? []).map(toSignalEvent), [arena?.recentDecisions]);
  const recentSignals = useMemo(() => latestPerSymbol(arena?.recentSignals ?? []).map(toSignalEvent), [arena?.recentSignals]);
  const [liveStreamEvents, setLiveStreamEvents] = useState<SignalEvent[]>(recentSignals);
  const [isPaused, setIsPaused] = useState(false);
  const [showHistoryModal, setShowHistoryModal] = useState(false);
  const symbols = useMemo(() => arena?.botSymbols ?? [], [arena?.botSymbols]);
  const decisionRows = useFeedWindow(symbols, recentDecisions);
  const signalRows = useFeedWindow(symbols, recentSignals, isPaused);

  useEffect(() => {
    if (!arena) setLiveStreamEvents([]);
    else if (!isPaused) setLiveStreamEvents(recentSignals);
  }, [arena, isPaused, recentSignals]);
        ...
        <div>
          <div className="flex items-center justify-between pb-2 mb-3 border-b border-[#2b3139]">
            <div>
              ...
              <p className="text-[11px] text-[#848e9c]">Motorun son kararları · emir sonucu değildir</p>
            </div>
            <div className="flex items-center gap-1 text-[11px] text-[#00d2ff] font-['JetBrains_Mono',monospace]">
              ...
              <span>{hasBackendAccount && arena?.latestDecisionAt && Date.now() - new Date(arena.latestDecisionAt).getTime() < 120_000 ? 'LIVE' : 'VERİ ESKİ'}</span>
            </div>
          </div>
          ...
          <div className="h-[454px] space-y-1.5 overflow-hidden">
            ...
            ) : <PendingFeedRow key={symbol} symbol={symbol} label="Karar bekleniyor" onSelectPair={onSelectPair} />)}
          </div>
        </div>
      ...
      >
        <div>
          ...
          <div className="h-[454px] space-y-1.5 overflow-hidden">
            ...
            ) : <PendingFeedRow key={symbol} symbol={symbol} label="Sinyal bekleniyor" onSelectPair={onSelectPair} />)}
          </div>
        </div>
