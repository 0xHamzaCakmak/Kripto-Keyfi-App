...
export const BalanceSourceCard: React.FC<BalanceSourceCardProps> = ({
  ...
  const isActive = hasBackendAccount && runningBots > 0 && arena?.analysisFresh === true;
  ...
  return (
    <div id="balance-source-card" className="h-full bg-[#1e2329]/80 border border-[#2b3139] rounded-xl p-5 shadow-xl flex flex-col justify-between">
      ...
      <div className="mt-5 pt-3 border-t border-[#2b3139]">
        ...
        <p className="text-[10px] text-[#848e9c] mb-2.5">{error || (!hasBackendAccount && mode === 'LIVE' ? 'Live API bağlı değil' : arena?.analysisFresh ? `${runningBots}/${totalBots} bot çalışıyor` : 'Yeni karar yok; kayıtlı bot durumu güncel olmayabilir.')}</p>
