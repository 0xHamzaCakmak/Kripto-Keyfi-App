...
export async function getNews(params: { q?: string; category?: string; tag?: string; topic?: string; cursor?: string; limit?: number; recentDays?: number } = {}) {
  const response = await api.get<ApiResponse<NewsList>>('/news', { params: { ...params, limit: params.limit ?? 18, category: params.category === 'Tümü' ? undefined : params.category } });
