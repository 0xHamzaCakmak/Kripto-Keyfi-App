...
export default function TradingBotDashboard() {
  ...
  return <div className="space-y-6">
    <header className="overflow-hidden rounded-[32px] border border-primary/15 bg-gradient-to-br from-surface via-surface to-primary/10 p-6 md:p-8">
      <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
        <div className="max-w-3xl">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-secondary/20 bg-secondary/10 px-3 py-1.5 text-xs font-black uppercase tracking-wider text-secondary"><ShieldCheck size={15}/> Faz 2 borsa hesapları hazır</div>
        ...
        <div className="grid gap-2 sm:grid-cols-2 xl:w-[390px]">
          <Status label="Bot motoru" value={data ? 'Yapılandırılmadı' : 'Kontrol ediliyor'} tone="neutral"/>
          <Status label="Canlı işlem" value="Kapalı" tone="safe"/>
          <Status label="Kill switch" value="Aktif" tone="safe"/>
    ...
    <div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
      ...
      <article className="rounded-[28px] border border-outline/10 bg-surface p-6">
        <div className="flex items-center gap-3"><div className="rounded-2xl bg-primary/10 p-3 text-primary"><Workflow size={22}/></div><div><p className="font-headline text-xl font-extrabold text-white">Faz 2 doğrulama listesi</p><p className="text-sm text-on-surface-variant">Credential girildiğinde tamamlanacak kontroller</p></div></div>
