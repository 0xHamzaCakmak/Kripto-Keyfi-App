...
function PreviewDialog({ quote, working, onClose, onApprove }: { quote: OrderPreview; working: boolean; onClose: () => void; onApprove: () => void }) { const asset = quoteAsset(quote.symbol); return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-background/85 p-4 backdrop-blur-sm"><section className="w-full max-w-xl rounded-[28px] border border-outline/10 bg-surface p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-primary">Son onay</p><h2 className="mt-2 font-headline text-2xl font-black text-white">Emir özeti</h2></div><button onClick={onClose} className="rounded-xl p-2 text-outline hover:bg-surface-high"><X/></button></div><dl className="mt-6 grid grid-cols-2 gap-3"><Summary label="Hesap" value={quote.accountName}/><Summary label="Parite" value={quote.symbol}/><Summary label="Yön / tip" value={`${quote.side} · ${quote.type}`}/><Summary label="Miktar" value={quote.quantity}/><Summary label="Mark fiyatı" value={quote.markPrice}/><Summary label="Tahmini büyüklük" value={`${quote.estimatedNotional} ${asset}`}/><Summary label="Kaldıraç / margin" value={`${quote.leverage}x · ${quote.marginMode}`}/><Summary label="Tahmini ilk teminat" value={`${quote.estimatedInitialMargin} ${asset}`}/></dl><div className="mt-4 space-y-2">{quote.warnings.map((warning) => <p key={warning} className="flex gap-2 rounded-xl bg-tertiary/5 p-3 text-xs text-on-surface-variant"><AlertTriangle size={15} className="shrink-0 text-tertiary"/>{warning}</p>)}</div><div className="mt-6 grid grid-cols-2 gap-3"><button onClick={onClose} disabled={working} className="rounded-xl border border-outline/15 px-4 py-3 text-sm font-bold text-on-surface-variant">Vazgeç</button><button onClick={onApprove} disabled={working} className="inline-flex items-center justify-center gap-2 rounded-xl bg-secondary px-4 py-3 text-sm font-black text-background disabled:opacity-50">{working ? <LoaderCircle className="animate-spin" size={17}/> : <CheckCircle2 size={17}/>} Onayla ve gönder</button></div></section></div>; }
