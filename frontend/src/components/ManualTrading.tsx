...
export default function ManualTrading() {
  ...
  async function approve() {
    if (!quote) return; setWorking(true); setError('');
    try {
      await confirmOrder(quote.id); setQuote(null); setNotice('Testnet/demo emri borsaya gönderildi. Açık Emirler ekranından takip edebilirsiniz.');
      setForm((current) => ({ ...current, quantity: '', price: '', stopPrice: '' }));
    } catch (reason) { setError(getApiErrorMessage(reason, 'Emir gönderilemedi.')); }
    finally { setWorking(false); }
  }
  ...
  return <div className="space-y-6">
    ...
    <form onSubmit={(event) => void createPreview(event)} className="grid gap-5 rounded-[28px] border border-outline/10 bg-surface p-5 md:p-7 xl:grid-cols-2">
      <Field label="Borsa hesabı"><select className="input" required value={accountId} onChange={(event) => setAccountId(event.target.value)}><option value="">Hesap seçin</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name} · {account.provider} {account.environment}{account.canTrade ? '' : ' · İŞLEM YETKİSİ YOK'}</option>)}</select></Field>
    ...
    {quote && <PreviewDialog quote={quote} working={working} onClose={() => setQuote(null)} onApprove={() => void approve()}/>} 
  </div>;
}

function PageHeader() { return <header className="rounded-[30px] border border-primary/15 bg-gradient-to-br from-surface to-primary/10 p-6 md:p-8"><div className="flex items-center gap-3 text-primary"><SlidersHorizontal/><span className="text-xs font-black uppercase tracking-[0.2em]">Trading Bot / Faz 3</span></div><h1 className="mt-3 font-headline text-3xl font-black text-white md:text-4xl">Manuel Testnet İşlemi</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-on-surface-variant">Borsa kurallarına göre doğrulanan emri önce inceleyin, ardından açıkça onaylayın. Bu ekran yalnızca testnet/demo hesaplarına emir gönderir.</p></header>; }
function PreviewDialog({ quote, working, onClose, onApprove }: { quote: OrderPreview; working: boolean; onClose: () => void; onApprove: () => void }) { return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-background/85 p-4 backdrop-blur-sm"><section className="w-full max-w-xl rounded-[28px] border border-outline/10 bg-surface p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-primary">Son onay</p><h2 className="mt-2 font-headline text-2xl font-black text-white">Emir özeti</h2></div><button onClick={onClose} className="rounded-xl p-2 text-outline hover:bg-surface-high"><X/></button></div><dl className="mt-6 grid grid-cols-2 gap-3"><Summary label="Hesap" value={quote.accountName}/><Summary label="Parite" value={quote.symbol}/><Summary label="Yön / tip" value={`${quote.side} · ${quote.type}`}/><Summary label="Miktar" value={quote.quantity}/><Summary label="Mark fiyatı" value={quote.markPrice}/><Summary label="Tahmini büyüklük" value={`${quote.estimatedNotional} USDT`}/><Summary label="Kaldıraç / margin" value={`${quote.leverage}x · ${quote.marginMode}`}/><Summary label="Tahmini ilk teminat" value={`${quote.estimatedInitialMargin} USDT`}/></dl><div className="mt-4 space-y-2">{quote.warnings.map((warning) => <p key={warning} className="flex gap-2 rounded-xl bg-tertiary/5 p-3 text-xs text-on-surface-variant"><AlertTriangle size={15} className="shrink-0 text-tertiary"/>{warning}</p>)}</div><div className="mt-6 grid grid-cols-2 gap-3"><button onClick={onClose} disabled={working} className="rounded-xl border border-outline/15 px-4 py-3 text-sm font-bold text-on-surface-variant">Vazgeç</button><button onClick={onApprove} disabled={working} className="inline-flex items-center justify-center gap-2 rounded-xl bg-secondary px-4 py-3 text-sm font-black text-background disabled:opacity-50">{working ? <LoaderCircle className="animate-spin" size={17}/> : <CheckCircle2 size={17}/>} Onayla ve gönder</button></div></section></div>; }
