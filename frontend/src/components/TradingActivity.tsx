...
export function OpenPositionsPage() {
  ...
  const [profileForm, setProfileForm] = useState<TradingExecutionProfile>({ minLeverage: 5, maxLeverage: 20, botAllocationUsdt: '500', minInitialMarginUsdt: '100', maxOrderNotional: '2000', maxInitialMargin: '500', maxAccountOpenNotional: '10000' });
  ...
  async function saveExecutionProfile() {
    ...
    try {
      ...
      setNotice('Merkezi bot sermayesi ve kaldıraç profili kaydedildi; aktif bot konfigürasyonları güncellendi. Açık pozisyonların mevcut kaldıracı değiştirilmez.');
...
function ExecutionField({ label, value, onChange, min = 1, max }: { label: string; value: string | number; onChange: (value: string) => void; min?: number; max?: number }) { return <label className="text-xs font-bold text-on-surface-variant">{label}<input type="number" min={min} max={max} step="1" value={value} onChange={(event) => onChange(event.target.value)} className="mt-1.5 w-full rounded-xl border border-outline/15 bg-background/70 p-3 text-white"/></label>; }
