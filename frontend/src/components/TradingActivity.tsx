  ...
  return <ActivityShell title="Açık Pozisyonlar" subtitle="Vadeli pozisyonlar borsadan okunur; kapatma emirleri her zaman reduce-only gönderilir." icon={TrendingUp} accounts={accounts} accountId={accountId} setAccountId={setAccountId} loading={loading} refresh={() => void load()} error={error} notice={notice} streamStatus={streamStatus}>
    ...
    {profile && <section className="mb-5 rounded-2xl border border-primary/20 bg-primary/5 p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <ExecutionField label="Asgari kaldıraç" value={profileForm.minLeverage} min={5} max={20} onChange={(value) => setProfileForm((current) => ({ ...current, minLeverage: Number(value) }))}/><ExecutionField label="Azami kaldıraç" value={profileForm.maxLeverage} min={5} max={20} onChange={(value) => setProfileForm((current) => ({ ...current, maxLeverage: Number(value) }))}/>
        <ExecutionField label="TESTNET stop-loss fiyat hareketi (%)" value={profileForm.stopLossBps / 100} min={0.5} max={10} step={0.1} onChange={(value) => setProfileForm((current) => ({ ...current, stopLossBps: Math.round(Number(value) * 100) }))}/><ExecutionField label="TESTNET net kâr hedefi (%)" value={profileForm.takeProfitBps / 100} min={1} max={50} step={0.1} onChange={(value) => setProfileForm((current) => ({ ...current, takeProfitBps: Math.round(Number(value) * 100) }))}/>
