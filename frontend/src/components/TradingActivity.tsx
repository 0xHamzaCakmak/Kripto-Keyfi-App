...
import { FileClock, LoaderCircle, RefreshCw, TrendingUp, Wifi, WifiOff } from 'lucide-react';
import { getApiErrorMessage } from '../services/apiClient';
import { cancelOrder, closeOpenPosition, getOpenOrders, getOpenPositions, getTradingAccounts, type OpenOrder, type OpenPosition, type TradingAccount } from '../services/tradingService';
...
export function OpenOrdersPage() {
  ...
  useEffect(() => {
    ...
    return subscribeTradingEvents(accountId, (event) => {
      ...
      if (event.topic === 'trading.order' || event.topic === 'trading.snapshot') void load();
    }, setStreamStatus);
  }, [accountId, load]);
...
export function OpenPositionsPage() {
  ...
  const [streamStatus, setStreamStatus] = useState<TradingStreamStatus>('OFFLINE');
  useEffect(() => { getTradingAccounts().then((items) => { setAccounts(items); setAccountId(items[0]?.id ?? ''); }).catch((reason) => setError(getApiErrorMessage(reason, 'Hesaplar alınamadı.'))); }, []);
  const load = useCallback(async () => { if (!accountId) { setRows([]); setLoading(false); return; } setLoading(true); setError(''); try { setRows(await getOpenPositions(accountId)); } catch (reason) { setError(getApiErrorMessage(reason, 'Pozisyonlar alınamadı.')); } finally { setLoading(false); } }, [accountId]);
  useEffect(() => { void load(); }, [load]);
  useEffect(() => {
    if (!accountId) { setStreamStatus('OFFLINE'); return; }
    return subscribeTradingEvents(accountId, (event) => {
      ...
      if (event.topic === 'trading.position' || event.topic === 'trading.account' || event.topic === 'trading.snapshot') void load();
    }, setStreamStatus);
  }, [accountId, load]);
  async function close(row: OpenPosition) { if (!window.confirm(`${row.symbol} ${row.side} pozisyonunun tamamı reduce-only piyasa emriyle kapatılsın mı?`)) return; setWorkingId(row.positionKey); setError(''); try { await closeOpenPosition(accountId, row); setNotice('Reduce-only kapatma emri gönderildi.'); await load(); } catch (reason) { setError(getApiErrorMessage(reason, 'Pozisyon kapatılamadı.')); } finally { setWorkingId(''); } }
