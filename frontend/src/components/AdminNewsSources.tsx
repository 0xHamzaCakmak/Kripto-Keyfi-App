...
export default function AdminNewsSources() {
  ...
  async function aiFixEditingArticle() {
    if (!editingArticle || retryingId) return;
    setRetryingId(editingArticle.id);
    setError('');
    try {
      const draft = await createNewsArticleAiDraft(editingArticle.id);
      setArticleForm({ titleTr: draft.titleTr, summaryTr: draft.summaryTr, whyItMatters: draft.whyItMatters, marketImpact: draft.marketImpact, watchOuts: draft.watchOuts, tags: draft.tags.join(', ') });
      showNotice(draft.needsReview ? 'AI taslağı hazır; kalite uyarıları nedeniyle dikkatle kontrol edin.' : 'AI taslağı hazır. Kontrol edip editoryal düzenlemeyi kaydedin.');
    } catch (reason) {
      setError(getApiErrorMessage(reason, 'AI metni düzeltemedi.'));
    } finally {
      setRetryingId(null);
    }
  }
  return (
    <div className="space-y-6">
      ...
      {editingArticle && <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/75 p-3 backdrop-blur-sm md:p-6"><form onSubmit={(event) => void saveArticleContent(event)} className="max-h-[92vh] w-full max-w-6xl space-y-4 overflow-y-auto rounded-3xl border border-secondary/25 bg-surface p-6 shadow-2xl"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-wider text-secondary">Editoryal düzenleme</p><h2 className="mt-1 font-headline text-2xl font-bold text-white">{editingArticle.originalTitle}</h2></div><button type="button" aria-label="Düzenleyiciyi kapat" onClick={() => setEditingArticle(null)} className="rounded-xl bg-surface-high p-2 text-on-surface"><XCircle/></button></div>{/\uFFFD|(?:Ã.|Â.|â(?:€|™|œ|ž)|Ä.|Å.)/.test(Object.values(articleForm).join(' ')) && <div className="rounded-2xl border border-error/30 bg-error/10 p-3 text-sm font-bold text-error">Metinde bozuk karakter kodlaması algılandı. “AI ile düzelt” seçeneğini kullanın veya karakterleri elle düzeltin.</div>}<label className="block text-sm font-bold text-on-surface-variant">Türkçe başlık<input value={articleForm.titleTr} onChange={(event) => setArticleForm({ ...articleForm, titleTr: event.target.value })} minLength={5} maxLength={500} required className="mt-2 w-full rounded-xl bg-surface-high px-4 py-3 text-white"/></label><label className="block text-sm font-bold text-on-surface-variant">Türkçe özet<textarea value={articleForm.summaryTr} onChange={(event) => setArticleForm({ ...articleForm, summaryTr: event.target.value })} minLength={20} maxLength={4000} required rows={6} className="mt-2 w-full rounded-xl bg-surface-high px-4 py-3 text-white"/></label><div className="grid gap-4 lg:grid-cols-3">{([['whyItMatters','Neden önemli?'],['marketImpact','Olası etkiler'],['watchOuts','Takip edilecekler']] as const).map(([key,label]) => <label key={key} className="text-sm font-bold text-on-surface-variant">{label}<textarea value={articleForm[key]} onChange={(event) => setArticleForm({ ...articleForm, [key]: event.target.value })} rows={5} className="mt-2 w-full rounded-xl bg-surface-high px-4 py-3 text-white"/></label>)}</div><label className="block text-sm font-bold text-on-surface-variant">Etiketler (virgülle ayırın)<input value={articleForm.tags} onChange={(event) => setArticleForm({ ...articleForm, tags: event.target.value })} className="mt-2 w-full rounded-xl bg-surface-high px-4 py-3 text-white"/></label><div className="flex flex-col gap-3 sm:flex-row"><button disabled={busy || Boolean(retryingId)} className="rounded-xl bg-secondary px-5 py-3 text-sm font-black text-background disabled:opacity-40">{busy ? 'Kaydediliyor…' : 'Editoryal düzenlemeyi kaydet'}</button><button type="button" disabled={busy || Boolean(retryingId)} onClick={() => void aiFixEditingArticle()} className="inline-flex items-center justify-center gap-2 rounded-xl border border-primary/40 bg-primary/10 px-5 py-3 text-sm font-black text-primary disabled:opacity-40"><RefreshCw size={16} className={retryingId === editingArticle.id ? 'animate-spin' : ''}/>{retryingId === editingArticle.id ? 'AI düzeltiyor…' : 'AI ile düzelt'}</button><p className="self-center text-xs text-on-surface-variant">AI sonucu forma gelir; kontrol edip kaydettiğiniz içerik worker tarafından değiştirilmez.</p></div></form></div>}
      {open && (
        <form
