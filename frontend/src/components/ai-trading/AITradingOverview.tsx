...
export default function AITradingOverview() {
  ...
  const toggleUniverse = async (symbol: string, enabled: boolean) => {
    ...
    try {
      ...
      setData((current) => current.universe ? { ...current, universe: { ...current.universe, assets: current.universe.assets.map((asset) => asset.symbol === symbol ? { ...updated, exchangeAvailable: updated.exchangeAvailable ?? asset.exchangeAvailable } : asset) } } : current);
    } catch (reason) { setError(getApiErrorMessage(reason, 'Trading Universe seçimi kaydedilemedi.')); }
    finally { setSavingSymbol(''); }
  };
  const addUniverse = async (symbol: string) => {
    setSavingSymbol(symbol); setError('');
    try {
      const added = await aiTradingApi.addTradingUniverseAsset(symbol);
      setData((current) => current.universe ? { ...current, universe: { ...current.universe,
        assets: [...current.universe.assets.filter((asset) => asset.symbol !== symbol), added].sort((left, right) => left.sortOrder - right.sortOrder),
      } } : current);
  ...
  return <AITradingPage title="AI Trading Operasyon Merkezi" description="Autonomous PAPER ve SHADOW sistemini, risk durumunu ve öğrenme döngüsünü tek güvenli görünümde izleyin." icon={Bot} action={<RefreshButton onClick={() => void load()} busy={loading} />}>
    ...
    {data.testnet?.connected && <section className="rounded-[24px] border border-secondary/20 bg-secondary/5 p-5"><div className="mb-4"><p className="text-xs font-black uppercase tracking-[.14em] text-secondary">Binance Demo performansı</p><p className="mt-1 text-xs text-on-surface-variant">Sabit başlangıç {formatMoney(Number(data.testnet.startingBalance))} · yalnız USD-M Futures USDT + USDC · açık PnL dahil</p></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Demo equity" value={formatMoney(Number(data.testnet.equity))} detail={`Stable cüzdan ${formatMoney(Number(data.testnet.totalBalance))}`} /><MetricCard label="Cüzdan farkı" value={formatMoney(Number(data.testnet.walletPnl))} detail="10.000 USD başlangıca göre" tone={Number(data.testnet.walletPnl) >= 0 ? 'safe' : 'danger'} /><MetricCard label="Açık PnL" value={formatMoney(Number(data.testnet.unrealizedPnl))} detail={`${data.testnet.openPositions} açık borsa pozisyonu`} tone={Number(data.testnet.unrealizedPnl) >= 0 ? 'safe' : 'danger'} /><MetricCard label="Toplam Demo net PnL" value={formatMoney(Number(data.testnet.netPnl))} detail={`${(data.testnet.pnlPercent * 100).toLocaleString('tr-TR', { maximumFractionDigits: 2 })}% · 10.000 USD başlangıca göre`} tone={Number(data.testnet.netPnl) >= 0 ? 'safe' : 'danger'} /></div></section>}
    ...
    <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
      ...
      <div className="space-y-5"><section className="rounded-[24px] border border-outline/10 bg-surface"><SectionHeader icon={Activity} title="Arena durumu" subtitle="Simulation-only runtime" /><div className="space-y-3 p-5"><StateRow label="Execution" value={data.arena?.executionMode ?? '—'} /><StateRow label="Son karar" value={formatDate(data.arena?.latestDecisionAt)} /><StateRow label="PAPER bot" value={String(data.arena?.modes.PAPER ?? 0)} /><StateRow label="SHADOW bot" value={String(data.arena?.modes.SHADOW ?? 0)} /></div></section><TradingUniversePanel universe={data.universe} savingSymbol={savingSymbol} onToggle={toggleUniverse} onAdd={addUniverse} /></div>
    </div>
...
function TradingUniversePanel({ universe, savingSymbol, onToggle, onAdd }: { universe: TradingUniverse | null; savingSymbol: string; onToggle: (symbol: string, enabled: boolean) => Promise<void>; onAdd: (symbol: string) => Promise<void> }) {
  const [query, setQuery] = useState('');
  const [results, setResults] = useState<TradingUniverseCandidate[]>([]);
  const [searching, setSearching] = useState(false);
  const [searchError, setSearchError] = useState('');
  const enabled = universe?.assets.filter((asset) => asset.enabled && asset.exchangeAvailable !== false).length ?? 0;
  const search = async () => {
    setSearching(true); setSearchError('');
    try { setResults((await aiTradingApi.searchTradingUniverse(query, 20)).results); }
    catch (reason) { setSearchError(getApiErrorMessage(reason, 'Binance Futures coin listesi aranamadı.')); setResults([]); }
    finally { setSearching(false); }
  };
  const add = async (symbol: string) => {
    try { await onAdd(symbol); setResults((current) => current.map((item) => item.symbol === symbol ? { ...item, listed: true, enabled: true } : item)); }
    catch { /* Parent displays the API error. */ }
  };
  return <section className="overflow-hidden rounded-[24px] border border-outline/10 bg-surface"><SectionHeader icon={Orbit} title="Trading Universe" subtitle={`${enabled}/${universe?.assets.length ?? 20} Futures coin aktif · yeni işlemler için`} />
    <form className="border-b border-outline/10 p-3" onSubmit={(event) => { event.preventDefault(); void search(); }}><div className="flex gap-2"><div className="relative min-w-0 flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-outline" size={15} /><input value={query} onChange={(event) => setQuery(event.target.value.toUpperCase())} placeholder="Binance Futures coin ara: BTC, TON…" className="w-full rounded-xl border border-outline/15 bg-background py-2.5 pl-9 pr-3 text-xs text-white outline-none focus:border-primary/50" /></div><button type="submit" disabled={searching} className="rounded-xl bg-primary px-3 text-xs font-black text-background disabled:opacity-50">{searching ? 'Aranıyor' : 'Ara'}</button></div>{searchError && <p className="mt-2 text-xs text-error">{searchError}</p>}{results.length > 0 && <div className="mt-3 max-h-52 space-y-1 overflow-y-auto rounded-xl bg-background/60 p-2">{results.map((item) => <div key={item.symbol} className="flex items-center justify-between gap-3 rounded-lg px-2 py-2 text-xs"><div><p className="font-black text-white">{item.baseAsset}<span className="ml-2 font-medium text-outline">{item.symbol}</span></p><p className="mt-0.5 text-[10px] text-outline">USDⓈ-M Perpetual · azami {item.maxLeverage}x</p></div><button type="button" disabled={item.enabled || savingSymbol === item.symbol} onClick={() => void add(item.symbol)} className="inline-flex items-center gap-1 rounded-lg border border-primary/20 px-2 py-1.5 font-black text-primary disabled:border-outline/10 disabled:text-outline"><Plus size={12} />{item.enabled ? 'Listede' : item.listed ? 'Yeniden ekle' : 'Ekle'}</button></div>)}</div>}</form>
    <div className="max-h-[430px] overflow-y-auto"><table className="w-full text-left text-xs"><thead className="sticky top-0 bg-surface-high text-outline"><tr><th className="p-3">Aktif</th><th className="p-3">Coin</th><th className="p-3 text-right">Futures</th><th className="p-3 text-right">24s hacim</th></tr></thead><tbody>{universe?.assets.map((asset) => <tr key={asset.symbol} className="border-t border-outline/10"><td className="p-3"><input aria-label={`${asset.symbol} işlemlerini etkinleştir`} type="checkbox" checked={asset.enabled} disabled={savingSymbol === asset.symbol || (asset.enabled === false && asset.exchangeAvailable === false)} onChange={(event) => void onToggle(asset.symbol, event.target.checked)} className="h-4 w-4 accent-primary" /></td><td className="p-3"><span className="font-bold text-white">{asset.baseAsset}</span><span className="ml-2 text-outline">{asset.displayName}</span><p className="mt-1 text-[10px] text-outline">{asset.symbol}{asset.marketRank ? ` · #${asset.marketRank}` : ''}</p></td><td className={`p-3 text-right font-bold ${asset.exchangeAvailable === true ? 'text-secondary' : asset.exchangeAvailable === false ? 'text-error' : 'text-outline'}`}>{asset.exchangeAvailable === true ? 'VAR' : asset.exchangeAvailable === false ? 'YOK' : 'KONTROL YOK'}</td><td className="p-3 text-right text-on-surface-variant">{compactUsd(asset.volume24h)}</td></tr>) ?? <tr><td className="p-4 text-on-surface-variant" colSpan={4}>Universe yükleniyor…</td></tr>}</tbody></table></div><p className="border-t border-outline/10 p-3 text-[11px] leading-5 text-outline">İşareti kaldırılan coinlerde yeni exposure Go Risk Engine tarafından reddedilir; mevcut açık pozisyonların SL/TP yönetimi sürer. Yeni coin yalnız bağlı {universe?.exchange?.name || 'Binance'} USDⓈ-M Futures kataloğunda doğrulanırsa eklenir. Geçmiş Memory kayıtları denetim izi olarak silinmez.</p></section>;
}
