...
export default function AITradingArena() {
  ...
  return <AITradingPage title="Bot Arena" description="Tüm autonomous PAPER ve Binance TESTNET botlarını gerçek backend kanıtlarıyla izleyin; bot satırına tıklayarak işlem geçmişini ve sermaye kotasını yönetin." icon={Bot} action={<RefreshButton onClick={() => void load()} busy={loading} />}>
    ...
    {loading ? <div className="h-72 animate-pulse rounded-[24px] bg-surface" /> : visibleRows.length === 0 ? <EmptyState title="Eşleşen bot yok" description="Backend henüz bot üretmemiş olabilir veya seçilen filtrelere uyan kanıt bulunmuyor." /> : <section className="overflow-hidden rounded-[24px] border border-outline/10 bg-surface"><div className="overflow-x-auto"><table className="w-full min-w-[1460px] text-left text-xs"><thead className="bg-surface-high uppercase text-outline"><tr>{['Bot', 'Pozisyon', 'Score', 'Toplam PnL', 'Açık PnL', 'ROI', 'PF', 'Max DD', 'İşlem/Fill', 'Win rate', 'Strategy', 'Status', 'Equity', ''].map((label) => <th key={label} className="p-3">{label}</th>)}</tr></thead><tbody>{visibleRows.map((row) => { const openPnl = row.operation?.position ? Number(row.operation.position.unrealizedPnl) : row.bot.paperPosition ? Number(row.bot.paperPosition.unrealizedPnl) : null; const equity = row.pnl === null ? null : Number(row.bot.startingPaperBalance) + row.pnl; return <tr key={row.bot.id} className="cursor-pointer border-t border-outline/10 hover:bg-surface-high/40" onClick={() => setSelected(row)}><td className="p-3"><p className="font-bold text-white">{row.bot.name}</p><div className="mt-1"><ModeBadge mode={row.bot.mode} /></div></td><Cell value={positionLabel(row.operation, row.bot)} compact /><Cell value={number(row.score?.score)} strong /><Cell value={formatMoney(row.pnl)} tone={row.pnl} /><Cell value={formatMoney(openPnl)} tone={openPnl} /><Cell value={formatPercent(row.roi)} tone={row.roi} /><Cell value={number(row.profitFactor)} /><Cell value={formatPercent(row.drawdown)} tone={row.drawdown === null ? null : -row.drawdown} /><Cell value={String(row.trades)} /><Cell value={formatPercent(row.winRate)} /><Cell value={row.bot.strategyVersion?.strategy.family ?? '—'} /><td className="p-3"><StatusBadge tone={statusTone(row.bot.lifecycleStatus)}>{row.bot.lifecycleStatus}</StatusBadge></td><Cell value={formatMoney(equity)} tone={row.pnl} /><td className="p-3"><button type="button" onClick={(event) => { event.stopPropagation(); setSelected(row); }} className="rounded-lg p-2 text-primary hover:bg-primary/10" aria-label={`${row.bot.name} detayını aç`}><ChevronRight size={18} /></button></td></tr>; })}</tbody></table></div></section>}
...
export function buildArenaRows(bots: AutonomousBot[], scores: LeaderboardRow[], summaries: TradeSummary[], champions: ChampionCandidate[], operations: TestnetBotOperation[] = []): ArenaRow[] {
  ...
  const operationMap = new Map(operations.map((item) => [item.botId, item])); return bots.map((bot) => {
    ...
    return { bot, score, summary, champion, operation, pnl, roi: pnl !== null && start > 0 ? pnl / start : null, profitFactor: operation ? null : summary?.profitFactor ?? null,
      drawdown: recordNumber(champion?.evidence, ['maxDrawdown']), trades: operation?.totalFills ?? bot._count.paperFills, winRate: completed > 0 ? (operation ? operation.wins : summary!.wins) / completed : null,
...
function positionLabel(operation: TestnetBotOperation | null, bot: AutonomousBot) { const position = operation?.position; if (position) return `${position.symbol} · ${position.side} · ${position.leverage}x`; if (operation) return 'Pozisyon arıyor'; const quantity = Number(bot.paperPosition?.netQuantity ?? 0); return quantity > 0 ? 'PAPER · LONG' : quantity < 0 ? 'PAPER · SHORT' : 'PAPER · FLAT'; }
function BotDrawer({ row, onClose, onActivated }: { row: ArenaRow; onClose: () => void; onActivated: () => Promise<void> }) {
  ...
  const realizedNet = operation ? Number(operation.netRealizedPnl) : paperPosition ? Number(paperPosition.realizedPnl) - Number(paperPosition.totalFees) : null;
...
function PaperFillHistory({ performance, loading }: { performance: PaperPerformance | null; loading: boolean }) { return <section className="mt-5 overflow-hidden rounded-[22px] border border-outline/10 bg-surface"><div className="p-5"><h3 className="font-headline font-black text-white">PAPER fill geçmişi</h3><p className="mt-1 text-xs text-outline">Simüle fill fiyatı, maliyet ve gerçekleşen PnL kayıtları.</p></div>{loading ? <div className="h-32 animate-pulse bg-surface-high" /> : !performance?.fills.length ? <div className="px-5 pb-5 text-sm text-on-surface-variant">Henüz PAPER fill yok.</div> : <div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left text-xs"><thead className="bg-surface-high uppercase text-outline"><tr>{['Zaman', 'Yön', 'Mark', 'Fill', 'Miktar', 'Notional', 'Realized PnL', 'Ücret'].map((label) => <th key={label} className="p-3">{label}</th>)}</tr></thead><tbody>{performance.fills.map((fill) => <tr key={fill.id} className="border-t border-outline/10"><td className="p-3 text-on-surface-variant">{formatDate(fill.occurredAt)}</td><td className={`p-3 font-bold ${fill.side === 'BUY' ? 'text-secondary' : 'text-error'}`}>{fill.side}</td><td className="p-3">{fill.markPrice}</td><td className="p-3 font-bold text-white">{fill.fillPrice}</td><td className="p-3">{fill.quantity}</td><td className="p-3">{formatMoney(Number(fill.notional))}</td><td className={Number(fill.realizedPnl) >= 0 ? 'p-3 font-bold text-secondary' : 'p-3 font-bold text-error'}>{formatMoney(Number(fill.realizedPnl))}</td><td className="p-3">{formatMoney(Number(fill.fee))}</td></tr>)}</tbody></table></div>}</section>; }
