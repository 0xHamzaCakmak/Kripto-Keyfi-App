...
import { aiTradingApi, botSymbols, recordNumber, type AutonomousBot, type ChampionCandidate, type LeaderboardRow, type MarketRegime, type TestnetBotOperation, type TradeSummary } from '../../services/aiTradingService';
...
export function buildArenaRows(bots: AutonomousBot[], scores: LeaderboardRow[], summaries: TradeSummary[], champions: ChampionCandidate[], operations: TestnetBotOperation[] = []): ArenaRow[] {
  ...
  const operationMap = new Map(operations.map((item) => [item.botId, item])); return bots.map((bot) => {
    ...
    const pnl = operation ? Number(operation.netRealizedPnl) + openPnl : summary ? Number(summary.totalPnl) : null; const start = operation?.allocationUsdt ?? Number(bot.startingPaperBalance); const completed = operation ? operation.wins + operation.losses : summary ? summary.wins + summary.losses : 0;
    return { bot, score, summary, champion, operation, pnl, roi: pnl !== null && start > 0 ? pnl / start : null, profitFactor: operation ? null : summary?.profitFactor ?? null,
      drawdown: recordNumber(champion?.evidence, ['maxDrawdown']), trades: operation?.totalFills ?? summary?.tradeCount ?? 0, winRate: completed > 0 ? (operation ? operation.wins : summary!.wins) / completed : null,
...
function positionLabel(operation: TestnetBotOperation | null) { const position = operation?.position; return position ? `${position.symbol} · ${position.side} · ${position.leverage}x` : operation ? 'Pozisyon arıyor' : 'PAPER'; }
function BotDrawer({ row, onClose, onActivated }: { row: ArenaRow; onClose: () => void; onActivated: () => Promise<void> }) {
  ...
  const [operation, setOperation] = useState<TestnetBotOperation | null>(row.operation); const [detailLoading, setDetailLoading] = useState(row.bot.mode === 'DEMO');
  ...
  useEffect(() => {
    ...
    aiTradingApi.testnetBotOperation(row.bot.id).then((value) => { if (active) setOperation(value.data); })
      .catch((reason) => { if (active) setActionError(getApiErrorMessage(reason, 'Gerçek TESTNET işlem geçmişi alınamadı.')); })
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/70" role="dialog" aria-modal="true" aria-label={`${row.bot.name} detayı`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside className="h-full w-full max-w-4xl overflow-y-auto border-l border-outline/15 bg-background p-6 shadow-2xl">
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-4"><MetricCard label="Toplam net PnL" value={formatMoney(row.pnl)} /><MetricCard label="Gerçekleşmiş net" value={operation ? formatMoney(Number(operation.netRealizedPnl)) : '—'} /><MetricCard label="Açık PnL" value={formatMoney(openPnl)} /><MetricCard label="ROI" value={formatPercent(operation?.position?.roi ?? row.roi)} /><MetricCard label="Giriş fill" value={operation?.entryFills ?? '—'} /><MetricCard label="Toplam fill" value={operation?.totalFills ?? row.trades} /><MetricCard label="Kapanış" value={operation?.closedFills ?? '—'} /><MetricCard label="Komisyon" value={operation ? formatMoney(Number(operation.commission)) : '—'} /></div>
