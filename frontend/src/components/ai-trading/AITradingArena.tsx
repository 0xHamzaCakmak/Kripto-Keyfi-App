...
function BotDrawer({ row, onClose, onActivated }: { row: ArenaRow; onClose: () => void; onActivated: () => Promise<void> }) {
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/70" role="dialog" aria-modal="true" aria-label={`${row.bot.name} detayı`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside className="h-full w-full max-w-4xl overflow-y-auto border-l border-outline/15 bg-background p-6 shadow-2xl">
      ...
      <section className={`${row.bot.mode === 'DEMO' ? 'mt-5' : 'hidden'} overflow-hidden rounded-[22px] border border-outline/10 bg-surface`}><div className="p-5"><h3 className="font-headline font-black text-white">Gerçekleşmiş Binance TESTNET fill kayıtları</h3><p className="mt-1 text-xs text-outline">Her satır ayrı bir Binance fill kaydıdır. Giriş/ekleme fill'lerinde realized PnL oluşmaz; net değer komisyonu içerir. Açık conditional emirler gösterilmez.</p></div>{detailLoading ? <div className="h-32 animate-pulse bg-surface-high" /> : !operation?.fills?.length ? <div className="px-5 pb-5 text-sm text-on-surface-variant">Henüz gerçekleşmiş fill yok.</div> : <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-xs"><thead className="bg-surface-high uppercase text-outline"><tr>{['Zaman', 'Sembol', 'Fill türü', 'Yön', 'Fiyat', 'Miktar', 'Notional', 'Realized PnL', 'Komisyon', 'Komisyon sonrası net'].map((label) => <th key={label} className="p-3">{label}</th>)}</tr></thead><tbody>{operation.fills.map((fill) => { const realized = Number(fill.realizedPnl); const net = Number(fill.netRealizedPnl); return <tr key={`${fill.symbol}-${fill.tradeId}`} className="border-t border-outline/10"><td className="p-3 text-on-surface-variant">{formatDate(fill.occurredAt)}</td><td className="p-3 font-bold text-white">{fill.symbol}</td><td className="p-3 text-on-surface-variant">{fill.reduceOnly ? 'KAPANIŞ' : 'GİRİŞ / EKLEME'} · {fill.orderType}</td><td className={`p-3 font-bold ${fill.side === 'BUY' ? 'text-secondary' : 'text-error'}`}>{fill.side}</td><td className="p-3 text-white">{fill.price}</td><td className="p-3 text-on-surface-variant">{fill.quantity}</td><td className="p-3 text-on-surface-variant">{formatMoney(Number(fill.quoteQuantity))}</td><td className={`p-3 font-bold ${pnlColor(realized)}`}>{fill.reduceOnly ? formatMoney(realized) : '—'}</td><td className="p-3 text-on-surface-variant">{fill.commission} {fill.commissionAsset}</td><td className={`p-3 font-black ${pnlColor(net)}`}>{formatMoney(net)}</td></tr>; })}</tbody></table></div>}</section>
...
function pnlColor(value: number) { return value > 0 ? 'text-secondary' : value < 0 ? 'text-error' : 'text-outline'; }
