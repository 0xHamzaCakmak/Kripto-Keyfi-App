  ...
  return <div className="space-y-6">
    <header className="rounded-[30px] border border-primary/15 bg-gradient-to-br from-surface via-surface to-primary/10 p-6 md:p-8">
      <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row"><Link to="/admin/trading/ai-pro?tab=bot-guide" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-outline/15 bg-surface-high px-5 py-3 font-bold text-white"><BookOpen size={19}/> Bot rehberi</Link><button type="button" onClick={() => setWizardOpen(true)} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-primary px-5 py-3 font-black text-background"><Plus size={19}/> {fixedType === 'GRID' ? 'Grid bot oluştur' : 'Yeni bot oluştur'}</button></div>
      </div>
    </header>
...
function BotWizard({ accounts, initialType, onClose, onCreated }: { accounts: TradingAccount[]; initialType?: 'GRID'; onClose: () => void; onCreated: (bot: TradingBot) => void }) {
  ...
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-background/85 p-4 backdrop-blur-sm"><div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[28px] border border-outline/15 bg-surface p-6 shadow-2xl">
    <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-wider text-primary">Adım {step} / 3</p><h2 className="mt-1 font-headline text-2xl font-black text-white">Bot oluşturma sihirbazı</h2><Link to="/admin/trading/ai-pro?tab=bot-guide" onClick={onClose} className="mt-2 inline-flex items-center gap-1.5 text-xs font-bold text-tertiary hover:text-white"><BookOpen size={14}/> Seçenekleri ayrıntılı öğren</Link></div><button type="button" onClick={onClose} className="rounded-xl p-2 text-on-surface-variant hover:bg-surface-high"><X/></button></div>
