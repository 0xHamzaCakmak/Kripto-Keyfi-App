...
tradingRouter.get('/manual-mentor-positions', validateRequest({ query: tradingAccountQuerySchema }), asyncHandler(mentorPositions));
tradingRouter.post('/manual-mentor-positions/:id/publish', validateRequest({ params: closePositionParamsSchema, body: publishMentorSignalBodySchema }), asyncHandler(publishMentor));
