...
export async function runChampionSelection(userId: string, config: ChampionSelectionConfig, ipAddress?: string) {
    ...
    for (const decision of decisions) {
      ...
      await tx.championCandidate.create({ data: {
        ...
        evidence: {
          ...
          regimeCoverage: decision.regimeCoverage, failedGates: decision.failedGates,
      ...
      if (!changed) continue;
      await tx.tradingBot.update({ where: { id: decision.botId }, data: { lifecycleStatus: decision.targetStatus, version: { increment: 1 } } });
  ...
  return { config, evaluated: decisions.length, decisions, liveActivated: false };
}
...
async function loadEvidence(userId: string) {
  return prisma.$queryRaw<EvidenceRow[]>(Prisma.sql`
    ...
    GROUP BY b.id, b.lifecycleStatus, latest.score, latest.totalTrades, latest.maxDrawdown, latest.metrics
  `);
}

function presentEvidence(row: EvidenceRow): PromotionEvidence {
  return {
    botId: row.botId, lifecycleStatus: row.lifecycleStatus, score: row.score?.toNumber() ?? null,
    ...
    regimeCoverage: Number(row.regimeCoverage),
  };
}
