...
const envSchema = z.object({
  ...
  AI_TRADING_LEARNING_INTERVAL_MINUTES: z.coerce.number().int().min(5).max(1440).default(5),
  AI_TRADING_LEARNING_MIN_NEW_TRADES: z.coerce.number().int().min(10).max(100_000).default(25),
  ...
  AI_TRADING_HEDGE_MODE_ENABLED: booleanString.default('false'),
  COINGECKO_API_KEY: z.string().trim().default(''),
  COINGECKO_API_BASE_URL: z.string().url().default('https://api.coingecko.com/api/v3'),
  COINMARKETCAP_API_KEY: z.string().trim().default(''),
  COINMARKETCAP_API_BASE_URL: z.string().url().default('https://pro-api.coinmarketcap.com/v1'),
