...
import { assessAutonomousHealth, type AutonomousHealthMetrics } from '../src/modules/ai-trading/autonomous-observability.service.js';
...
function metrics(overrides: Partial<AutonomousHealthMetrics> = {}): AutonomousHealthMetrics {
  return {
    ...
    marketData: { latestObservedAt: new Date(), lagMs: 1_000 }, strategyExecution: { averagePersistenceLatencyMs: 4 },
...
describe('autonomous observability', () => {
  it('reports healthy, degraded and emergency-stop states deterministically', () => {
    ...
    expect(assessAutonomousHealth(metrics({ marketData: { latestObservedAt: null, lagMs: null } }))).toBe('DEGRADED');
